import { useState, useEffect } from "react";
import Input from "../ui/Input";
import Button from "../ui/Button";
import { addExperience, updateExperience } from "../../services/profileService";

const ExperienceForm = ({ existingData, onClose, onSuccess }) => {
    const [formData, setFormData] = useState({
        role: "",
        companyName: "",
        location: "",
        startDate: "",
        endDate: "",
        currentlyWorking: false,
    });

    useEffect(() => {
        if (existingData) {
            const expItem = Array.isArray(existingData) ? existingData[0] : existingData;
            setFormData({
                role: expItem.role || "",
                companyName: expItem.companyName || "",
                location: expItem.location || "",
                startDate: expItem.startDate ? expItem.startDate.split("T")[0] : "",
                endDate: expItem.endDate ? expItem.endDate.split("T")[0] : "",
                currentlyWorking: expItem.currentlyWorking || false,
            });
        } else {
            setFormData({
                role: "",
                companyName: "",
                location: "",
                startDate: "",
                endDate: "",
                currentlyWorking: false,
            });
        }
    }, [existingData]);

    const handleChange = (e) => {
        const { name, value, type, checked } = e.target;
        setFormData((prev) => ({
            ...prev,
            [name]: type === "checkbox" ? checked : value,
        }));
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if (existingData?._id) {
                await updateExperience(existingData._id, formData);
            } else {
                await addExperience(formData);
            }
            onSuccess?.();
            onClose?.();
        } catch (err) {
            console.error(err);
        }
    };

    return (
        <form onSubmit={handleSubmit} className="flex flex-col h-full text-gray-900 dark:text-gray-100">

            <h2 className="text-lg font-semibold mb-4">
                {existingData ? "Edit Experience" : "Add Experience"}
            </h2>

            <Input
                label="Role *"
                name="role"
                value={formData.role}
                onChange={handleChange}
                required
            />

            <Input
                label="Company Name *"
                name="companyName"
                value={formData.companyName}
                onChange={handleChange}
                required
            />

            <Input
                label="Location"
                name="location"
                value={formData.location}
                onChange={handleChange}
            />

            <div className="grid grid-cols-2 gap-4">
                <Input
                    label="Date of Joining"
                    type="date"
                    name="startDate"
                    value={formData.startDate}
                    onChange={handleChange}
                />

                {!formData.currentlyWorking && (
                    <Input
                        label="Date of Leaving"
                        type="date"
                        name="endDate"
                        value={formData.endDate}
                        onChange={handleChange}
                    />
                )}
            </div>

            <div className="mt-3">
                <label className="text-sm flex items-center gap-2">
                    <input
                        type="checkbox"
                        name="currentlyWorking"
                        checked={formData.currentlyWorking}
                        onChange={handleChange}
                        className="accent-blue-500 dark:accent-blue-400"
                    />
                    Currently working in this role
                </label>
            </div>

            <div className="flex justify-end mt-6 gap-3">
                <Button
                    type="button"
                    onClick={onClose}
                    className="bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600"
                >
                    Cancel
                </Button>
                <Button
                    type="submit"
                    className="bg-blue-50 dark:bg-blue-900 text-blue-500 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-800"
                >
                    {existingData ? "Update" : "Save"}
                </Button>
            </div>
        </form>
    );
};

export default ExperienceForm;
