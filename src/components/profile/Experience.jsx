...
const Experience = ({ data = [], onAdd, refreshProfile }) => {
    ...
    return (
        <Card className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 transition-colors duration-300">
            <SectionHeader title="Experience" onAdd={onAdd} />
                ...
                data.map((item) => (
                    ...
                    >
                        <div className="flex gap-4">
                            ...
                            {/* Text info */}
                            <div>
                                <p className="font-medium text-gray-900 dark:text-gray-100">{item.role}</p>
                                <p className="text-sm text-gray-700 dark:text-gray-300">{item.companyName}</p>
                        ...
                        <div className="relative" ref={dropdownRef}>
                            ...
                            {openId === item._id && (
                                <div className="absolute lg:left-0 right-0 mt-2 w-28 bg-white dark:bg-gray-700 shadow-lg border border-gray-100 dark:border-gray-600 py-2 z-50 rounded-md transition-colors duration-300">
                                    <button
                                        className="flex items-center gap-2 px-4 py-2 text-xs w-full text-left text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200"
                                        onClick={() => {
                                            onAdd(item);
                                            setOpenId(null);
                                        }}
                                    ...
                                    <button
                                        className="flex items-center gap-2 px-4 py-2 text-xs w-full text-left text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900 transition-colors duration-200"
